...
const SeatSelectionScreen = () => {
  ...
  const {busId} = route.params as {busId: any};
  ...
  useFocusEffect(useCallback(() => refetch, [busId]));

  if (isLoading) {
    return (
      <View className="flex-1 busInfos-center justify-center">
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="bg-white p-4 flex-row busInfos-center border-b-[1px] border-teal-400">
        <TouchableOpacity>
          ...
          <View className="flex-grow gap-2 mt-3">
            {busInfo?.badges?.map((badge: string, index: number) => (
              <View
                ...
                <Text className="text-xs text-yello-800 font-semibold">
                  {badge}
                </Text>
              </View>
            ))}
